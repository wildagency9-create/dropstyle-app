...
app.post('/api/tarifs/materiaux', verifyToken, async (req, res) => {
    try {
        ...
        await conn.query(
            'INSERT INTO materiaux (user_id, support, price, categorie) VALUES (?, ?, ?, ?)',
            [req.userId, support, price, categorie]
        );
        await conn.release();
...
app.put('/api/tarifs/vinyles/:id', verifyToken, async (req, res) => {
    try {
        ...
        const conn = await pool.getConnection();
        await conn.query(
            'UPDATE vinyles SET name = ?, price = ?, type = ? WHERE id = ? AND user_id = ?',
            [name, price, type, req.params.id, req.userId]
        );
        await conn.release();
...
app.post('/api/devis', verifyToken, async (req, res) => {
    try {
        ...
        const conn = await pool.getConnection();
        await conn.query(
            'INSERT INTO devis (user_id, type, qty, ht, ttc, details, created_at) VALUES (?, ?, ?, ?, ?, ?, NOW())',
            [req.userId, type, qty, ht, ttc, JSON.stringify(details)]
        );
        await conn.release();
...
app.get('/api/devis', verifyToken, async (req, res) => {
    try {
        ...
        const [rows] = await conn.query(
            'SELECT * FROM devis WHERE user_id = ? ORDER BY created_at DESC LIMIT 50',
            [req.userId]
        );
        await conn.release();
        res.json(rows);
